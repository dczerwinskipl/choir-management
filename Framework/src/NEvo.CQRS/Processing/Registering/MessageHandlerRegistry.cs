...
{
    private readonly ConcurrentMultivalueDictionary<Type, IMessageHandlerAdapter> _handlers = new();
    ...
    public IMessageHandlerAdapter GetHandler<TMessage, TResult>(TMessage message) where TMessage : IMessage<TResult> => GetHandlers<TMessage, TResult>(message).Single();
    ...
    public void Register<THandler>()
    {
        ...
        foreach (var handlerDescription in messageTypes)
        {
            ...
            _handlers.Add(handlerDescription.MessageType, messageHandlerOptions.MessageHandlerAdapterFactory.Create(handlerDescription, _serviceProvider));
        }
    }
    ...
    protected IEnumerable<MessageHandlerDescription> GetHandlersDescriptions(Type handlerType) =>
        handlerType
            ...
            .SelectMany(handlerInterface => _messageHandlerOptions[handlerInterface.GetGenericTypeDefinition()].MessageHandlerAdapterFactory.GetMessageHandlerDescriptions(handlerType, handlerInterface));
}
/*
 handlerInterface =>
